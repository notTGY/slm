...
def main(max_steps=-1, num_samples=1000, batch_size=4, seq_len=512, epochs=1, base_model="mikeoxmaul/zmeeust-baby-l"):
    os.environ["TOKENIZERS_PARALLELISM"] = "false"
    tokenizer = AutoTokenizer.from_pretrained(base_model)
    tokenizer.pad_token_id = tokenizer.eos_token_id
    tokenizer.chat_template = chat_template

    dataset = Sharegpt(tokenizer, num_samples=num_samples, max_length=seq_len)
    ...
    checkpoint_callback = ModelCheckpoint(
        dirpath="checkpoints/",
        filename="llama-sharegpt-{step:06d}",
    ...
    model.model.save_pretrained(f"hf-checkpoints/llama-sharegpt-{trainer.global_step:06d}")
    tokenizer.save_pretrained(f"hf-checkpoints/llama-sharegpt-{trainer.global_step:06d}")
