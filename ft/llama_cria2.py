...
def main(max_steps=-1, num_samples=23941, batch_size=32, seq_len=65, epochs=1, base_model="mikeoxmaul/zmeeust-baby-l"):
    os.environ["TOKENIZERS_PARALLELISM"] = "false"
    tokenizer = AutoTokenizer.from_pretrained("EleutherAI/gpt-neo-125M")
    tokenizer.pad_token_id = tokenizer.eos_token_id
    tokenizer.chat_template = chat_template

    dataset = Cria(tokenizer, num_samples=num_samples, max_length=seq_len)
    ...
    model.model.save_pretrained(f"hf-checkpoints/llama-cria2-{trainer.global_step:06d}")
    tokenizer.save_pretrained(f"hf-checkpoints/llama-cria2-{trainer.global_step:06d}")
