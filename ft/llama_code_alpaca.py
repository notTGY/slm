...
class CodeAlpaca(Dataset):
    def __init__(
        ...
        super().__init__()
        self.ds = load_dataset("HuggingFaceH4/CodeAlpaca_20K", streaming=True)
        self.dataset = list(self.ds["train"].take(num_samples))
        eos_id = tokenizer.eos_token_id

        self.data = []
        for d in self.dataset:
            prompt_messages = [{"role": "user", "content": d["prompt"]}]
            full_messages = [
...
def main(max_steps=-1, num_samples=18019, batch_size=32, seq_len=128, epochs=1, base_model="mikeoxmaul/zmeeust-baby-l"):
    os.environ["TOKENIZERS_PARALLELISM"] = "false"
    tokenizer = AutoTokenizer.from_pretrained(base_model)
    tokenizer.pad_token_id = tokenizer.eos_token_id
    tokenizer.chat_template = chat_template

    dataset = CodeAlpaca(tokenizer, num_samples=num_samples, max_length=seq_len)
